...
from Loader import MultiStyleDataset
from util import *
import scipy.misc
...
parser.add_argument(
    '--contentImageFile',
    default='images/content/la_chatte.png',
    help='path to train')
parser.add_argument(
    '--stylePath',
...
parser.add_argument(
    '--outf', default='samples/', help='folder to output images')
parser.add_argument(
    '--alpha',
    type=float,
    default=1,
...
dataset = MultiStyleDataset(args.contentImageFile, args.stylePath, args.fineSize)
...
def wct2(cF, sF):
    ...
    contentConv = torch.mm(
        cF, cF.t()).div(cFSize[1] - 1) + torch.eye(cFSize[0]).double()
    ...
    targetFeature = torch.mm(
        torch.mm(
            torch.mm(s_v[:, 0:k_s], torch.diag(s_d)), (s_v[:, 0:k_s].t())),
        whiten_cF)
...
print('Processed %d images. Averaged time is %f' % ((i + 1),
                                                    avgTime / (i + 1)))
